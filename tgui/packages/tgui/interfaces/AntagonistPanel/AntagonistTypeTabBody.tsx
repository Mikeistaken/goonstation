...
export const AntagonistTypeTabBody = (props: AntagonistPanelData) => (
  <Box>
    {props.currentTabSections ? (
      props.currentTabSections.map((section, index) => <AntagonistTabSection key={index} {...section} />)
    ) : (
    ...
    <Stack justify="space-around">
      <Stack.Item grow>
        <Section title="Antagonist Mortality Rate">
          ...
          />
          <LabeledList>
            <LabeledList.Item label="Alive Antagonists">{props.mortalityRates.antagonistsAlive}</LabeledList.Item>
            <LabeledList.Item label="Dead Antagonists">{props.mortalityRates.antagonistsDead}</LabeledList.Item>
          </LabeledList>
        </Section>
      </Stack.Item>
      <Stack.Item grow>
        <Section title="Crew Mortality Rate">
          ...
          />
          <LabeledList>
            <LabeledList.Item label="Alive Crew">{props.mortalityRates.crewAlive}</LabeledList.Item>
            <LabeledList.Item label="Dead Crew">{props.mortalityRates.crewDead}</LabeledList.Item>
          </LabeledList>
        </Section>
      </Stack.Item>
    </Stack>
...
const TableAntagonistEntry = (props: AntagonistData, context) => {
  ...
  const sortSubordinateAntagonists
    = data.subordinateAntagonists[props.antagonist_datum]?.sort((a, b) => a.real_name.localeCompare(b.real_name)) || [];
  ...
  return (
    <>
      <Table.Row>
            ...
            <Box inline>
              {!!props.has_subordinate_antagonists && (
                <Button
                  ...
                  my={-0.5}
                  mr={0.8}
                  icon={data.subordinateAntagonists[antagonist_datum] ? 'chevron-down' : 'chevron-right'}
                  onClick={() => act(
                    `${
        ...
        <TablePositionCell mind_ref={props.mind_ref} {...data.mindLocations[props.mind_ref]} />
...
const AntagonistRoleTooltip = (props) => {
  ...
  return <LabeledList.Item label="Antagonist Role">{toTitleCase(display_name)}</LabeledList.Item>;
};
...
const AdminPMButton = (props, context) => {
  ...
  const { act } = useBackend<AntagonistPanelData>(context);
  return <Button content="PM" onClick={() => act('admin_pm', { mind_ref })} tooltip="Admin PM" mr={1} />;
};
...
const NuclearBombReadout = (props: TabSectionData, context) => {
  ...
  return (
    <Section title={toTitleCase(sectionName)}>
      <Stack align="center">
        <Stack.Item>
          <Box textAlign="center" textColor="red" fontFamily="Consolas" fontSize={5} m={0.5} mr={2}>
...
const TableHeadEntry = (props: HeadsData, context) => {
  ...
  return (
    <Table.Row>
      ...
      <TablePositionCell mind_ref={mind_ref} {...data.mindLocations[mind_ref]} />
      <Table.Cell>
        <AdminPMButton {...props} />
        <PlayerOptionsButton {...props} />
      </Table.Cell>
