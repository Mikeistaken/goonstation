...
export const AntagonistTypeTabs = (props, context) => {
  ...
  return (
    <Section inline fill>
      <Tabs vertical>
        <Tabs.Tab selected={tabIndex === null} onClick={() => SetTab(null)} width={12} my={0.2}>
          General
        </Tabs.Tab>
        <Box my={-0.5}>
          <Divider />
        ...
        {sortTabs.map((tab, index) => (
          <Tabs.Tab key={index} selected={tabIndex === tab.index} onClick={() => SetTab(tab.index)} width={12} my={0.2}>
